...
class NFLApp {
  ...
  hideAllViews() {
    ...
    const teamScheduleContainer = document.getElementById(
      ...
      if (refreshedData) {
        ...
        if (this.currentRoute.week === currentWeek || !this.currentRoute.week) {
          if (this.currentRoute.view === 'scoreboard') {
            ...
            this.currentRoute.view === 'game-detail' &&
            this.currentRoute.gameId
          ) {
  ...
  async refreshCurrentWeek() {
    ...
    try {
      const refreshedData = await this.apiService.refreshWeekData(
        this.currentRoute.week
      );
      if (refreshedData) {
